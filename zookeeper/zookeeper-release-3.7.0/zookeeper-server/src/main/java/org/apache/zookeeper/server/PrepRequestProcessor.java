...
public class PrepRequestProcessor extends ZooKeeperCriticalThread implements RequestProcessor {
    ...
    public PrepRequestProcessor(ZooKeeperServer zks, RequestProcessor nextProcessor) {
        ...
        this.nextProcessor = nextProcessor; // nextProcessor 是  SyncRequestProcessor
        this.zks = zks;
    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                request.prepStartTime = Time.currentElapsedTime();//请求开始处理时间
                /**
                 * PrepRequestProcessor处理 请求，主要包括
                 * 1、acl校验
                 * 2、生成并记录ChangeRecord
                 * .....其他.....
                 */
    ...
    protected void addChangeRecord(ChangeRecord c) {
        /**
         * 在PreRequestProcessor中并没有将 ChangeRecord 持久化
         */
        synchronized (zks.outstandingChanges) {
    ...
    protected void pRequest2Txn(int type, long zxid, Request request, Record record, boolean deserialize) throws KeeperException, IOException, RequestProcessorException {
        ...
        /**
         * 重点是进行一些权限校验、创建节点的修改记录 ChangeRecord(事务日志)，但未持久化，持久化在SyncRequestProcessor处理器中
         */
        ...
        case OpCode.setData:
            ...
            setTxnDigest(request, nodeRecord.precalculatedDigest);
            /**
             * 添加 ChangeRecord，底层只是将其添加到了 ZooKeeperServer 对象的 outstandingChanges队列中了
             * 代表了一个写请求;在最后FinalRequestProcessor处理器最终要执行修改操作时用到了这个outstandingChanges队列；
             * 可以查看 FinalRequestProcessor.applyRequest函数，最终在ZooKeeperServer.processTxn函数中
             */
            addChangeRecord(nodeRecord);
            break;
    ...
    protected void pRequest(Request request) throws RequestProcessorException {
        ...
        if (!request.isThrottled()) {
            /**
             * PreRequestProcessor处理过程
             * 主要是权限校验,按照请求type生成事务日志记录等操作
             */
          pRequestHelper(request);
        }

        request.zxid = zks.getZxid();//获取并设置请求的事务id 读请求获取的还是最后一次的事务id,写请求才会增长id
        ...
        /**
         * 调用下一个处理器进行处理
         * nextProcessor是SyncRequestProcessor，也是一个单独的线程
         */
    ...
    /**
     * This method is a helper(助手) to pRequest method
     *
     * @param request
     */
    private void pRequestHelper(Request request) throws RequestProcessorException {
        try {
            /**
             * 根据请求的类型进行不同的处理
             * 总之会发现在PreRequestProcessor中，有的类型直接放过该请求进入请求处理器链的下一个环节，有的则是根据请求类型不同生成新的请求对象
             * 然后大都调用了pRequest2Txn方法进行处理，该方法中进行一些权限校验，添加数据修改记录(事务日志相关;但还未持久化事务) 等其他的事，还并未涉及数据处理
             * 真正的数据处理在处理器链的下一个环节
             */
            switch (request.type) {
                /**
                 * 需要记录事务日志的请求，先生成事务信息
                 * 非常重要：
                 * 像 get 等请求则不会生成事务日志信息,这些请求对应的request对象中,缺失两个信息;
                 *      private TxnHeader hdr; //事务头信息
                 *      private Record txn;   //事务数据
                 *  成为SyncRequestProcessor处理器判断是否需要持久化事务的依据
                 */
            case OpCode.createContainer:
            case OpCode.create:
            case OpCode.create2: //create命令
                CreateRequest create2Request = new CreateRequest();  // Record的实现 是一个数据容器
                //为请求创建事务信息(数据变更记录等信息)  zks.getNextZxid()则获取了事务id zxid
            ...
            //All the rest don't need to create a Txn - just verify session  看注释 一下这些操作不需要创建 ChangeRecord 事务操作日志 即不会产生事务
            case OpCode.sync:
            case OpCode.exists:
            case OpCode.getData:
            case OpCode.getACL:
    ...
    public void processRequest(Request request) {
        ...
        /**
         * 将请求Request添加到PreRequestProcessor的处理队列中
         * 由当前线程不断去消费
         * PreRequestProcessor的处理逻辑查看run方法
         */
