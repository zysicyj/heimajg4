...
public class FileTxnLog implements TxnLog, Closeable {
    ...
    public synchronized boolean append(TxnHeader hdr, Record txn, TxnDigest digest) throws IOException {
        if (hdr == null) {
            /**
             * 对于读请求譬如：get stat 等会走到这里,不会产生真正的事务日志
             */
            return false;
        }
        ...
        if (logStream == null) {
            ...
            // Make sure that the magic number is written before padding. 魔数
            logStream.flush();
     ...
     */
    /**
     * 事务日志刷盘,将FileOutputStream中的数据持久化到磁盘
     * @throws IOException
     */
    public synchronized void commit() throws IOException {
        ...
        for (FileOutputStream log : streamsToFlush) {
            ...
            if (forceSync) {
                ...
                FileChannel channel = log.getChannel();//Java nio file channel
                channel.force(false);
