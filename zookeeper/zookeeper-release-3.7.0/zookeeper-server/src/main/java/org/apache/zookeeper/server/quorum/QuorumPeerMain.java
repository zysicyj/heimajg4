...
public class QuorumPeerMain {
    ...
    public void runFromConfig(QuorumPeerConfig config) throws IOException, AdminServerException {
        ...
        try {
            ...
            /**
             * 创建并配置网络通信组件
             * 创建并配置AcceptThread,SelectorThreads
             * 但并未启动，这是跟单机版不一样的地方
             * 集群需要先选举出leader，完成数据同步后才启动网络组件对外提供服务
             */
            ...
            quorumPeer.enableLocalSessionsUpgrading(config.isLocalSessionsUpgradingEnabled());//会话升级
            //quorumPeer.setQuorumPeers(config.getAllMembers());
            //设置选举算法类型  config.getElectionAlg()=3  基于 TCP 的 FastLeaderElection
